...
export default function AdminPortal() {
  ...
  const [activeTab, setActiveTab] = useState<'DASHBOARD' | 'INVENTORY' | 'ORDERS' | 'APPEARANCE'>('ORDERS');
  ...
  const handleSaveCatalogue = async () => {
    ...
    if (editingCatalogue) { await supabase.from('catalogues').update({ name: newCatName }).eq('id', editingCatalogue.id); } 
    else { await supabase.from('catalogues').insert([{ name: newCatName }]); }
    setNewCatName(''); setEditingCatalogue(null); fetchData();
  };

  const cancelEdit = () => {
    setEditingProduct(null); setPName(''); setPPrice(''); setVariants([]); setCatId('');
  };
  ...
  const handleVideoUpload = async (e: any) => {
    try {
      ...
      if (!file) return;
      setIsUploading(true);
      const fileName = `bg_hero_${Math.random()}.mp4`;
      await supabase.storage.from('products').upload(fileName, file);
      const { data: { publicUrl } } = supabase.storage.from('products').getPublicUrl(fileName);
      await supabase.from('site_settings').upsert({ key: 'bg_video', value: publicUrl }, { onConflict: 'key' });
      alert("Video Updated");
      fetchData();
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen text-white font-sans relative">
      ...
      {bgVideo ? (
        <video src={bgVideo} autoPlay loop muted playsInline className="fixed inset-0 -z-10 w-full h-full object-cover opacity-20 grayscale" />
      ) : (
