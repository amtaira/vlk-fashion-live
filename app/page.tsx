  ...
  return (
    <main className="min-h-screen text-white relative overflow-x-hidden font-sans">
      ...
      <div className="fixed bottom-8 left-8 z-[100] mix-blend-difference pointer-events-none">
        <img src="/vlogo.png" alt="ONR Branding" className="w-16 h-auto opacity-80" />
      </div>
      ...
      <nav className="fixed top-0 w-full z-[100] px-6 py-8 flex justify-between items-center mix-blend-difference">
        ...
        {/* Fixed Navbar Brand Header */}
        <button onClick={() => navigate('HOME')} className="text-sm md:text-lg font-black uppercase tracking-widest text-white hover:text-red-600 transition-colors bg-transparent border-none outline-none cursor-pointer">
          Outfit Not Random
        </button>

        <button onClick={() => setIsCartOpen(true)} className="relative cursor-pointer bg-transparent border-none outline-none">
      ...
      <div className={`fixed inset-0 z-[150] transition-opacity duration-500 ${isMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className={`relative w-80 h-full bg-black/40 backdrop-blur-xl border-r border-white/10 p-10 flex flex-col justify-center space-y-8 transition-transform duration-500 ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          {['CATALOGUE', 'SIZE_GUIDE', 'SUSTAINABILITY', 'FOUNDATION', 'SHIPMENT', 'COOKIE'].map((id) => (
            <button key={id} onClick={() => navigate(id as ViewState)} className="text-2xl font-black uppercase italic text-left hover:text-red-600 transition-colors bg-transparent border-none cursor-pointer text-white">
              {id.replace('_', ' ')}
            </button>
          ))}
        </div>
      ...
      <div className="relative z-10">
        {view === 'HOME' && (
          <div className="min-h-screen flex flex-col items-center justify-center transform -translate-y-10">
            <button onClick={() => navigate('CATALOGUE')} className="px-12 py-4 border border-white hover:bg-red-600 hover:border-red-600 transition-all uppercase font-black text-xs tracking-[0.4em] text-white cursor-pointer z-20 relative">
              Shop With Us
            </button>
          </div>
        )}
        ...
        {view === 'SHIPMENT' && (
          <PolicyPage title="Shipment Policy">
            <p>Outfit Not Random Logistics ensures worldwide shipping within 7-14 business days.</p>
        ...
        {view === 'SUSTAINABILITY' && (
          <PolicyPage title="Sustainability">
            <p>We are committed to zero-waste production. Every garment architecture piece is crafted from 100% heavy-weight organic cotton and recycled fibers.</p>
            <p>Our packaging is 100% biodegradable, aligning with our vision of high-end design structures that respect the earth.</p>
          </PolicyPage>
        )}
          ...
          <div className="max-w-6xl mx-auto grid grid-cols-2 lg:grid-cols-3 gap-10 mt-40 px-6">
            {activeCat?.products?.map((p: any) => (
              <div key={p.id} onClick={() => navigate('DETAIL', activeCat, p)} className="cursor-pointer bg-black/40 p-4 rounded-2xl border border-white/5 backdrop-blur-sm">
                <img src={p.image_url} alt={`${p.name} - Outfit Not Random`} className="aspect-[3/4] object-contain mb-4" />
        ...
        {view === 'DETAIL' && selectedProduct && (
          <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 bg-black/60 p-10 rounded-[40px] border border-white/10 backdrop-blur-xl mt-40 mx-6">
            <img src={activeImage} alt={`Outfit Not Random - ${selectedProduct.name}`} className="w-full h-auto object-contain rounded-3xl" />
            <div className="space-y-8">
              ...
              <div className="flex gap-2">
                {['S', 'M', 'L', 'XL', 'XXL'].map(s => (
                  <button key={s} onClick={() => setSelectedSize(s)} className={`w-12 h-12 border font-black ${selectedSize === s ? 'bg-red-600 border-red-600' : 'border-white/20'} text-white`}>{s}</button>
                ))}
              </div>
        ...
        <div className="fixed inset-0 z-[200] flex justify-end">
          ...
          <div className="relative w-full max-w-md bg-black border-l border-white/10 h-full p-8 flex flex-col">
            <div className="flex justify-between items-center mb-10">
              ...
              <button onClick={() => setIsCartOpen(false)} className="text-[10px] uppercase opacity-50 text-white bg-transparent border-none cursor-pointer">Continue Shopping</button>
            </div>
                ...
                <div key={item.cartId} className="flex gap-4 items-center border-b border-white/5 pb-4">
                  ...
                  <div className="flex-1 text-[10px] font-black uppercase">
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      <button onClick={() => updateQuantity(item.cartId, -1)} className="w-5 h-5 border border-white/20 flex items-center justify-center text-white bg-transparent">-</button>
                  ...
                  <div className="text-right">
                    ...
                    <button onClick={() => removeFromCart(item.cartId)} className="text-red-600 text-[10px] bg-transparent border-none cursor-pointer">Remove</button>
                  </div>
                </div>
              ))}
            </div>
            <div className="pt-6 border-t border-white/10">
              ...
              <button onClick={() => { setShowPaymentModal(true); setIsCartOpen(false); }} className="w-full py-5 bg-red-600 font-black uppercase text-white border-none cursor-pointer">Proceed to Payment</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-black border border-white/10 p-8 rounded-3xl">
            <div className="flex justify-between items-center mb-8">
              ...
              <button onClick={() => { setShowPaymentModal(false); setIsCartOpen(true); }} className="text-red-600 text-[10px] font-black uppercase underline bg-transparent border-none cursor-pointer">Back to Bag</button>
            </div>
            <div className="flex flex-wrap gap-2 mb-8 border-b border-white/10 pb-4">
              {['MPESA_STK', 'MPESA_PAYBILL', 'VISA'].map((m) => (
                <button key={m} onClick={() => setPaymentMethod(m as any)} className={`text-[10px] font-black px-4 py-2 rounded-full border cursor-pointer ${paymentMethod === m ? 'bg-red-600 border-red-600 text-white' : 'border-white/10 opacity-50 text-white'}`}>
                  {m.replace('_', ' ')}
                </button>
              ))}
            </div>
            {paymentMethod === 'MPESA_STK' && (
              <div className="space-y-4">
                <input type="text" placeholder="2547XXXXXXXX" className="w-full bg-white/5 border border-white/10 p-4 rounded-xl outline-none text-white" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} />
                <button onClick={submitOrder} className="w-full py-4 bg-red-600 text-white font-black uppercase rounded-xl border-none cursor-pointer">Send Prompt</button>
              </div>
            )}
            {paymentMethod === 'MPESA_PAYBILL' && (
              <div className="space-y-4 bg-white/5 p-6 rounded-2xl border border-white/10">
                ...
                <button onClick={submitOrder} className="w-full py-4 bg-red-600 mt-4 font-black uppercase text-white border-none cursor-pointer">Confirm Payment</button>
              </div>
            )}
            {paymentMethod === 'VISA' && (
              <div className="space-y-4">
                <input type="text" placeholder="Card Number" className="w-full bg-white/5 border border-white/10 p-4 rounded-xl outline-none text-white" value={cardNumber} onChange={(e) => setCardNumber(e.target.value)} />
                <div className="grid grid-cols-2 gap-4">
                   <input type="text" placeholder="MM/YY" className="w-full bg-white/5 border border-white/10 p-4 rounded-xl outline-none text-white" value={cardExpiry} onChange={(e) => setCardExpiry(e.target.value)} />
                   <input type="text" placeholder="CVV" className="w-full bg-white/5 border border-white/10 p-4 rounded-xl outline-none text-white" value={cardCvv} onChange={(e) => setCardCvv(e.target.value)} />
                </div>
                <button onClick={submitOrder} className="w-full py-4 bg-red-600 text-white font-black uppercase rounded-xl border-none cursor-pointer">Complete with Visa</button>
              </div>
            )}
          </div>
        </div>
      ...
      <footer className="py-10 text-center opacity-20 text-[8px] uppercase tracking-[0.5em] font-black">
        © 2026 Outfit Not Random / Architectural Archive
      </footer>
    </main>
  );
}