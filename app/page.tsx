  ...
  return (
    <main className="min-h-screen text-white relative overflow-x-hidden font-sans">
      ...
      <video 
        ...
        muted 
        playsInline 
        className="fixed inset-0 -z-25 w-full h-full object-cover object-center opacity-60 pointer-events-none" 
      />

      <div 
        className="fixed inset-0 -z-20 w-full h-full bg-cover bg-center opacity-10 pointer-events-none"
        style={{ backgroundImage: "url('/hero-big.jpg')" }} 
      />
      
      {(view === 'GRID' || view === 'DETAIL') && (
        <div 
          className="fixed inset-0 -z-10 opacity-30 bg-cover bg-center pointer-events-none transition-opacity duration-1000"
      ...
      <div className="pt-32 px-6 pb-20 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 max-w-5xl mx-auto">
            {catalogues.map(cat => (
              <div key={cat.id} onClick={() => navigate('GRID', cat)} className="h-96 border border-white/10 rounded-3xl flex items-center justify-center cursor-pointer hover:bg-white/5 transition-all group overflow-hidden relative">
                ...
                <h2 className="text-3xl font-black uppercase tracking-widest group-hover:text-red-600 z-10 transition-colors">{cat.name}</h2>
              </div>
            ))}
          </div>
        )}
          ...
          <div className="max-w-6xl mx-auto grid grid-cols-2 lg:grid-cols-3 gap-10">
            {activeCat?.products?.map((p: any) => (
              <div key={p.id} onClick={() => navigate('DETAIL', activeCat, p)} className="cursor-pointer group text-center bg-black/40 p-4 rounded-2xl border border-white/5 backdrop-blur-sm">
                ...
                <p className="text-red-600 font-bold mt-2">£{p.price}.00</p>
              </div>
            ))}
          </div>
        )}
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-2">
                  {['S', 'M', 'L', 'XL', 'XXL'].map(s => (
                    <button key={s} onClick={() => setSelectedSize(s)} className={`w-12 h-12 border font-black transition-all ${selectedSize === s ? 'bg-red-600 text-white border-red-600' : 'border-white/20 hover:border-white'}`}>{s}</button>
                  ))}
                </div>
              </div>
              <button onClick={addToBag} className="w-full py-5 bg-white text-black font-black uppercase tracking-widest hover:bg-red-600 hover:text-white transition-all">Add to Bag</button>
            </div>
          </div>
        )}
        ...
        {view === 'SUSTAINABILITY' && (
          <ContentPage title="Sustainability">
            ...
            <p>Our fabrics are sourced from 100% recycled cotton fleece and organic textiles.</p>
          </ContentPage>
        )}
          ...
          <div className="relative w-full max-w-lg bg-black border border-white/10 p-8 rounded-3xl overflow-y-auto max-h-[90vh]">
            ...
            <div className="flex gap-2 mb-8 border-b border-white/10 pb-4">
              {['MPESA_STK', 'MPESA_PAYBILL', 'VISA'].map((m) => (
                <button key={m} onClick={() => setPaymentMethod(m as any)} className={`text-[10px] font-black px-4 py-2 rounded-full border ${paymentMethod === m ? 'bg-red-600 text-white border-red-600' : 'border-white/10 opacity-50'}`}>
                  {m.replace('_', ' ')}
                </button>
              ))}
            </div>
            {paymentMethod === 'MPESA_STK' && (
              <div className="space-y-4">
                <input type="text" placeholder="2547..." className="w-full bg-white/5 border border-white/10 p-4 rounded-xl outline-none focus:border-red-600" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} />
                <button onClick={submitOrder} className="w-full py-4 bg-red-600 text-white font-black uppercase rounded-xl">Send STK Prompt</button>
              </div>
            )}
          </div>
        </div>
      ...
      {isCartOpen && (
        <div className="fixed inset-0 z-[200] flex justify-end">
          ...
          <div className="relative w-full max-w-md bg-black border-l border-white/10 h-full p-8 flex flex-col">
            <h2 className="font-black uppercase tracking-widest mb-10 text-red-600">Your Bag</h2>
            ...
            <button onClick={() => setShowPaymentModal(true)} className="w-full py-5 bg-red-600 text-white font-black uppercase mt-4 hover:bg-white hover:text-black transition-all">Proceed to Payment</button>
          </div>
        </div>
      )}
    </main>
